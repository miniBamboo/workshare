...
func (gl GasLimit) IsValid(parentGasLimit uint64) bool {
	gasLimit := uint64(gl)
	if gasLimit < workshare.MinGasLimit {
		return false
	}
	var diff uint64
	...
	return diff <= parentGasLimit/workshare.GasLimitBoundDivisor
}
...
func (gl GasLimit) Qualify(parentGasLimit uint64) uint64 {
	gasLimit := uint64(gl)
	maxDiff := parentGasLimit / workshare.GasLimitBoundDivisor
	if gasLimit > parentGasLimit {
...
func (gl GasLimit) Adjust(delta int64) uint64 {
	gasLimit := uint64(gl)
	maxDiff := gasLimit / workshare.GasLimitBoundDivisor
	...
	// reduce
	diff := min64(uint64(-delta), maxDiff)
	if workshare.MinGasLimit+diff > gasLimit {
		// reach floor
		return workshare.MinGasLimit
	}
	return gasLimit - diff
}
