...
func init() {
	defines := []struct {
		...
		{"native_get", func(env *xenv.Environment) []interface{} {
			...
			bal, err := Energy.Native(env.State(), env.BlockContext().Time).Get(workshare.Address(addr))
			if err != nil {
				panic(err)
			}
		...
		{"native_add", func(env *xenv.Environment) []interface{} {
			...
			exist, err := env.State().Exists(workshare.Address(args.Addr))
			if err != nil {
				panic(err)
			}
			if exist {
			...
			if err := Energy.Native(env.State(), env.BlockContext().Time).Add(workshare.Address(args.Addr), args.Amount); err != nil {
				panic(err)
			}
			return nil
		}},
		{"native_sub", func(env *xenv.Environment) []interface{} {
			...
			env.UseGas(workshare.GetBalanceGas)
			ok, err := Energy.Native(env.State(), env.BlockContext().Time).Sub(workshare.Address(args.Addr), args.Amount)
			if err != nil {
				panic(err)
			}
			if ok {
		...
		{"native_master", func(env *xenv.Environment) []interface{} {
			...
			env.UseGas(workshare.GetBalanceGas)
			master, err := env.State().GetMaster(workshare.Address(addr))
			if err != nil {
				panic(err)
			}
