...
func approverEvent(approver workshare.Address, action string) *tx.Event {
	...
	return &tx.Event{
		...
		Topics:  []workshare.Bytes32{ev.ID(), workshare.BytesToBytes32(approver.Bytes())},
		Data:    data,
	}
}
func proposalEvent(id workshare.Bytes32, action string) *tx.Event {
	...
	return &tx.Event{
		Address: builtin.Executor.Address,
		Topics:  []workshare.Bytes32{ev.ID(), id},
		Data:    data,
	}
}

func votingContractEvent(addr workshare.Address, action string) *tx.Event {
	...
	return &tx.Event{
		Address: builtin.Executor.Address,
		Topics:  []workshare.Bytes32{ev.ID(), workshare.BytesToBytes32(addr.Bytes())},
		Data:    data,
	}
}
...
func initExectorTest() *ctest {
	...
	b0 := buildGenesis(db, func(state *state.State) error {
		...
		builtin.Params.Native(state).Set(workshare.KeyExecutorAddress, new(big.Int).SetBytes(builtin.Executor.Address[:]))
		return nil
	})
	...
	rt := runtime.New(chain, st, &xenv.BlockContext{Time: uint64(time.Now().Unix())}, workshare.NoFork)
...
func TestExecutorApprover(t *testing.T) {
	...
	var approvers []workshare.Address
	for i := 0; i < 7; i++ {
		approvers = append(approvers, workshare.BytesToAddress([]byte(fmt.Sprintf("approver%d", i))))
	}

	for _, a := range approvers {
		// zero identity
		test.Case("addApprover", a, workshare.Bytes32{}).
			ShouldVMError(errReverted).
			Assert(t)

		test.Case("addApprover", a, workshare.BytesToBytes32(a.Bytes())).
	...
	test.Case("addApprover", approvers[0], workshare.BytesToBytes32(approvers[0].Bytes())).
		Caller(builtin.Executor.Address).
		ShouldVMError(errReverted).
		Assert(t)
	...
	for _, a := range approvers {
		test.Case("approvers", a).
			ShouldOutput(workshare.BytesToBytes32(a.Bytes()), true).
			Assert(t)
	}
...
func TestExecutorVotingContract(t *testing.T) {
	...
	test := initExectorTest()
	voting := workshare.BytesToAddress([]byte("voting"))
...
func TestExecutorProposal(t *testing.T) {
	...
	data, _ := setParam.EncodeInput(workshare.BytesToBytes32([]byte("paramKey")), big.NewInt(12345))
	test.Case("propose", target, data).
		ShouldVMError(errReverted).
		Assert(t)

	approver := workshare.BytesToAddress([]byte("approver"))
	test.Case("addApprover", approver, workshare.BytesToBytes32(approver.Bytes())).
		Caller(builtin.Executor.Address).
		Assert(t)

	proposalID := func() workshare.Bytes32 {
		var b8 [8]byte
		binary.BigEndian.PutUint64(b8[:], test.rt.Context().Time)
		return workshare.Bytes32(crypto.Keccak256Hash(b8[:], approver[:]))
	}()
	test.Case("propose", target, data).
		Caller(approver).
	...
	assert.Equal(t, M(big.NewInt(12345), nil), M(builtin.Params.Native(test.rt.State()).Get(workshare.BytesToBytes32([]byte("paramKey")))))
}
