...
func (c *ccase) To(to workshare.Address) *ccase {
	c.to = to
	return c
}

func (c *ccase) Caller(caller workshare.Address) *ccase {
	c.caller = caller
	return c
}
...
func (c *ccase) TxID(txID workshare.Bytes32) *ccase {
	c.txID = txID
	return c
}
...
func (c *ccase) GasPayer(gasPayer workshare.Address) *ccase {
	c.gasPayer = gasPayer
	return c
}
...
func TestParamsNative(t *testing.T) {
	executor := workshare.BytesToAddress([]byte("e"))
	db := muxdb.NewMem()
	b0 := buildGenesis(db, func(state *state.State) error {
		state.SetCode(builtin.Params.Address, builtin.Params.RuntimeBytecodes())
		builtin.Params.Native(state).Set(workshare.KeyExecutorAddress, new(big.Int).SetBytes(executor[:]))
		return nil
	})
	...
	rt := runtime.New(chain, st, &xenv.BlockContext{}, workshare.NoFork)
	...
	setEvent := func(key workshare.Bytes32, value *big.Int) *tx.Event {
		ev, _ := builtin.Params.ABI.EventByName("Set")
		data, _ := ev.Encode(value)
		return &tx.Event{
			Address: builtin.Params.Address,
			Topics:  []workshare.Bytes32{ev.ID(), key},
			Data:    data,
		}
	}
...
func TestAuworkshareityNative(t *testing.T) {
	var (
		master1   = workshare.BytesToAddress([]byte("master1"))
		endorsor1 = workshare.BytesToAddress([]byte("endorsor1"))
		identity1 = workshare.BytesToBytes32([]byte("identity1"))

		master2   = workshare.BytesToAddress([]byte("master2"))
		endorsor2 = workshare.BytesToAddress([]byte("endorsor2"))
		identity2 = workshare.BytesToBytes32([]byte("identity2"))

		master3   = workshare.BytesToAddress([]byte("master3"))
		endorsor3 = workshare.BytesToAddress([]byte("endorsor3"))
		identity3 = workshare.BytesToBytes32([]byte("identity3"))
		executor  = workshare.BytesToAddress([]byte("e"))
	)
	...
	b0 := buildGenesis(db, func(state *state.State) error {
		...
		state.SetBalance(workshare.Address(endorsor1), workshare.InitialProposerEndorsement)
		state.SetCode(builtin.Params.Address, builtin.Params.RuntimeBytecodes())
		builtin.Params.Native(state).Set(workshare.KeyExecutorAddress, new(big.Int).SetBytes(executor[:]))
		builtin.Params.Native(state).Set(workshare.KeyProposerEndorsement, workshare.InitialProposerEndorsement)
		return nil
	})
	repo, _ := chain.NewRepository(db, b0)
	st := state.New(db, b0.Header().StateRoot(), 0, 0, 0)
	...
	candidateEvent := func(nodeMaster workshare.Address, action string) *tx.Event {
		ev, _ := builtin.Auworkshareity.ABI.EventByName("Candidate")
		var b32 workshare.Bytes32
		copy(b32[:], action)
		data, _ := ev.Encode(b32)
		return &tx.Event{
			Address: builtin.Auworkshareity.Address,
			Topics:  []workshare.Bytes32{ev.ID(), workshare.BytesToBytes32(nodeMaster[:])},
			Data:    data,
		}
	}
	...
	test := &ctest{
		rt:     rt,
		abi:    builtin.Auworkshareity.ABI,
		to:     builtin.Auworkshareity.Address,
		caller: executor,
	}
	...
	test.Case("first").
		ShouldOutput(workshare.Address{}).
		Assert(t)
	...
	test.Case("add", master1, endorsor1, identity1).
		Caller(workshare.BytesToAddress([]byte("other"))).
		ShouldVMError(errReverted).
		Assert(t)
	...
	test.Case("revoke", master2).
		Caller(workshare.BytesToAddress([]byte("some one"))).
		Assert(t)
...
func TestEnergyNative(t *testing.T) {
	var (
		addr   = workshare.BytesToAddress([]byte("addr"))
		to     = workshare.BytesToAddress([]byte("to"))
		master = workshare.BytesToAddress([]byte("master"))
		eng    = big.NewInt(1000)
	)
	...
	transferEvent := func(from, to workshare.Address, value *big.Int) *tx.Event {
		ev, _ := builtin.Energy.ABI.EventByName("Transfer")
		data, _ := ev.Encode(value)
		return &tx.Event{
			Address: builtin.Energy.Address,
			Topics:  []workshare.Bytes32{ev.ID(), workshare.BytesToBytes32(from[:]), workshare.BytesToBytes32(to[:])},
			Data:    data,
		}
	}
	approvalEvent := func(owner, spender workshare.Address, value *big.Int) *tx.Event {
		ev, _ := builtin.Energy.ABI.EventByName("Approval")
		data, _ := ev.Encode(value)
		return &tx.Event{
			Address: builtin.Energy.Address,
			Topics:  []workshare.Bytes32{ev.ID(), workshare.BytesToBytes32(owner[:]), workshare.BytesToBytes32(spender[:])},
			Data:    data,
		}
	}

	rt := runtime.New(chain, st, &xenv.BlockContext{Time: b0.Header().Timestamp()}, workshare.NoFork)
	test := &ctest{
		rt:     rt,
	...
	test.Case("transferFrom", addr, workshare.BytesToAddress([]byte("some one")), big.NewInt(10)).
		Caller(to).
		ShouldLog(transferEvent(addr, workshare.BytesToAddress([]byte("some one")), big.NewInt(10))).
		ShouldOutput(true).
		Assert(t)
...
func TestPrototypeNative(t *testing.T) {
	var (
		acc1 = workshare.BytesToAddress([]byte("acc1"))
		acc2 = workshare.BytesToAddress([]byte("acc2"))

		master    = workshare.BytesToAddress([]byte("master"))
		notmaster = workshare.BytesToAddress([]byte("notmaster"))
		user      = workshare.BytesToAddress([]byte("user"))
		notuser   = workshare.BytesToAddress([]byte("notuser"))

		credit       = big.NewInt(1000)
		recoveryRate = big.NewInt(10)
		sponsor      = workshare.BytesToAddress([]byte("sponsor"))
		notsponsor   = workshare.BytesToAddress([]byte("notsponsor"))

		key      = workshare.BytesToBytes32([]byte("account-key"))
		value    = workshare.BytesToBytes32([]byte("account-value"))
		contract workshare.Address
	)
	...
	st.SetStorage(workshare.Address(acc1), key, value)
	st.SetBalance(workshare.Address(acc1), big.NewInt(1))

	masterEvent := func(self, newMaster workshare.Address) *tx.Event {
		ev, _ := builtin.Prototype.Events().EventByName("$Master")
		data, _ := ev.Encode(newMaster)
		return &tx.Event{
			Address: self,
			Topics:  []workshare.Bytes32{ev.ID()},
			Data:    data,
		}
	}

	creditPlanEvent := func(self workshare.Address, credit, recoveryRate *big.Int) *tx.Event {
	...
	userEvent := func(self, user workshare.Address, action string) *tx.Event {
		...
		return &tx.Event{
			Address: self,
			Topics:  []workshare.Bytes32{ev.ID(), workshare.BytesToBytes32(user.Bytes())},
			Data:    data,
		}
	}

	sponsorEvent := func(self, sponsor workshare.Address, action string) *tx.Event {
		...
		return &tx.Event{
			Address: self,
			Topics:  []workshare.Bytes32{ev.ID(), workshare.BytesToBytes32(sponsor.Bytes())},
			Data:    data,
		}
	}
	...
	test.Case("storageFor", acc1, key).
		ShouldOutput(value).
		Assert(t)
	test.Case("storageFor", acc1, workshare.BytesToBytes32([]byte("some-key"))).
		ShouldOutput(workshare.Bytes32{}).
		Assert(t)

	// should be hash of rlp raw
	expected, err := st.GetStorage(builtin.Prototype.Address, workshare.Blake2b(contract.Bytes(), []byte("credit-plan")))
	assert.Nil(t, err)
	test.Case("storageFor", builtin.Prototype.Address, workshare.Blake2b(contract.Bytes(), []byte("credit-plan"))).
...
func TestPrototypeNativeWithLongerBlockNumber(t *testing.T) {
	...
	rt := runtime.New(chain, st, &xenv.BlockContext{
		Number: workshare.MaxStateHistory + 1,
...
func TestExtensionNative(t *testing.T) {
	...
	gasPayer := workshare.BytesToAddress([]byte("gasPayer"))
	...
	rt := runtime.New(chain, st, &xenv.BlockContext{Number: 2, Time: b2.Header().Timestamp(), TotalScore: b2.Header().TotalScore(), Signer: b2_singer}, workshare.NoFork)
	...
	test.Case("blake2b256", []byte("hello world")).
		ShouldOutput(workshare.Blake2b([]byte("hello world"))).
		Assert(t)
	...
	test.Case("txID").
		TxID(workshare.BytesToBytes32([]byte("txID"))).
		ShouldOutput(workshare.BytesToBytes32([]byte("txID"))).
		Assert(t)
