...
func NewSchedulerV1(
	...
	if !listed {
		return nil, errors.New("unauworkshareized block proposer")
	}
...
func (s *SchedulerV1) Schedule(nowTime uint64) (newBlockTime uint64) {
	const T = workshare.BlockInterval
...
func (s *SchedulerV1) IsTheTime(newBlockTime uint64) bool {
	...
	if (newBlockTime-s.parentBlockTime)%workshare.BlockInterval != 0 {
		// invalid block time
		return false
	}
...
func (s *SchedulerV1) Updates(newBlockTime uint64) (updates []Proposer, score uint64) {
	...
	toDeactivate := make(map[workshare.Address]Proposer)

	t := newBlockTime - workshare.BlockInterval
	for i := uint64(0); i < workshare.MaxBlockProposers && t > s.parentBlockTime; i++ {
		p := s.whoseTurn(t)
		if p.Address != s.proposer.Address {
			toDeactivate[p.Address] = p
		}
		t -= workshare.BlockInterval
	}
...
func dprp(blockNumber uint32, time uint64) uint64 {
	...
	return binary.BigEndian.Uint64(workshare.Blake2b(b4[:], b8[:]).Bytes())
}
