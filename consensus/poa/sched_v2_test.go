...
func TestSchedulerV2_Updates(t *testing.T) {
	...
	tests := []struct {
		...
		{"p1 should not deactivate others", fields{
			Proposer{p1, true},
			parentTime,
			[]workshare.Address{p1, p2, p3, p4, p5},
...
func TestNewSchedulerV2(t *testing.T) {
	seed := workshare.Bytes32{}.Bytes()
	parentNumber := uint32(10)

	/*
		var ordered []workshare.Address
		var list []struct {
			addr workshare.Address
			hash workshare.Bytes32
		}
		var addrs = []workshare.Address{p1, p2, p3, p4, p5}
		var num [4]byte
		...
		for _, addr := range addrs {
			list = append(list, struct {
				addr workshare.Address
				hash workshare.Bytes32
			}{addr, workshare.Blake2b(seed, num[:], addr.Bytes())})
		}
	...
	*/
	...
	tests := []struct {
	...
	}{
		{
			...
			&SchedulerV2{Proposer{p1, true}, parentTime, []workshare.Address{p1, p4, p3, p2, p5}},
			false,
		},
		{
			...
			&SchedulerV2{Proposer{p1, false}, parentTime, []workshare.Address{p1, p4, p3, p2, p5}},
			false,
		},
		{
