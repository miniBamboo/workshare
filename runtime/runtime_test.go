...
func TestContractSuicide(t *testing.T) {
	...
	addr := workshare.BytesToAddress([]byte("acc01"))
	...
	exec, _ := runtime.New(repo.NewChain(b0.Header().ID()), state, &xenv.BlockContext{Time: time}, workshare.NoFork).
	...
	expectedEvent := &tx.Event{
		...
		Topics:  []workshare.Bytes32{event.ID(), workshare.BytesToBytes32(addr.Bytes()), workshare.BytesToBytes32(origin.Bytes())},
...
func TestChainID(t *testing.T) {
	...
	exec, _ := runtime.New(repo.NewChain(b0.Header().ID()), state, &xenv.BlockContext{}, workshare.ForkConfig{ETH_IST: 0}).
		PrepareClause(tx.NewClause(&addr).WithData(methodData), 0, math.MaxUint64, &xenv.TransactionContext{})
	out, _, err := exec()
	assert.Nil(t, err)
	assert.Nil(t, out.VMErr)

	assert.Equal(t, g.ID(), workshare.BytesToBytes32(out.Data))
}
...
func TestBlake2(t *testing.T) {
	...
	assert.Equal(t, workshare.MustParseBytes32("ba80a53f981c4d0d6a2797b69f12f6e94c212f14685ac4b74b12bb6fdbffa2d1"), workshare.Bytes32(hashes[0]))
	assert.Equal(t, workshare.MustParseBytes32("7d87c5392aab792dc252d5de4533cc9518d38aa8dbf1925ab92386edd4009923"), workshare.Bytes32(hashes[1]))
}
...
func TestCall(t *testing.T) {
	...
	rt := runtime.New(repo.NewChain(b0.Header().ID()), state, &xenv.BlockContext{}, workshare.NoFork)
	...
	assert.Equal(t, workshare.Address(addr), genesis.DevAccounts()[0].Address)
...
func TestExecuteTransaction(t *testing.T) {
	...
	// addr1 := workshare.Address(crypto.PubkeyToAddress(key.PublicKey))
	// addr2 := workshare.BytesToAddress([]byte("acc2"))
	...
	// 	workshare.Address{}, 0, 0, 0, func(uint32) workshare.Bytes32 { return workshare.Bytes32{} })
